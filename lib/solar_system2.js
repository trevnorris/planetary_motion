...
class SolarSystem {
  ...
  init() {
    const M = this[sun_s].mass;

    // First calculate and set the initial velocity of each planet.
    for (let b of this[bodies_s]) {
      const bi = b.inicon;
      const bv = Math.sqrt(G * M * (2 / bi.aphelion - 1 / bi.semi_major));
      const vel = b.velocity();
      vel.y = bv * Math.cos(bi.inclination * Math.PI / 180);
      vel.z = -bv * Math.sin(bi.inclination * Math.PI / 180);
    }
...
/* debug:start */
this[bodies_s].unshift(this[sun_s]);
/* debug:stop */
  ...
  step(t) {
    ...
    for (let i = 0; i < len; i++) {
      //add_acceleration(this[bodies_s][i], sun);
      for (let j = i + 1; j < len; j++) {
        add_acceleration(this[bodies_s][i], this[bodies_s][j]);
      }
      add_position_velocity(this[bodies_s][i], t, sun);
    }
    //add_position_velocity(sun);
